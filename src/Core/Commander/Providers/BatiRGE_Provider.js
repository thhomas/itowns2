...
define('Core/Commander/Providers/BatiRGE_Provider',[
    ...
    BatiRGE_Provider.prototype.generateMesh = function(longitude,latitude,radius)
    {
       ...
       function createElements(elements) {
            ...
            console.log("EELLLLLLLLLLLLLLLLLLL ",elements);
            
            //createGeometry3DfromPoly(data)
              ...
              for( var r = 0 ; r <features.length ; r++){
                    ...
                    var polygon       = features[r].geometry.coordinates[0][0];
                     ...
                     if(polygon.length > 2){
                                ...
                                for(var j=0; j< polygon.length -1; ++j){
                                   ...
                                   var coordCarto1 = new CoordCarto().setFromDegreeGeo(p1.z  ,p1.x, altitude_sol);
                                   var coordCarto2 = new CoordCarto().setFromDegreeGeo(p1.z  ,p1.x, altitude_sol + hauteur);// + Math.random(1000) );
                 ...
                 //**************** ROOF ****************************
                               ...
                               triangles.forEach(function(t) {
                                   ...
                                   var coordCarto1 = new CoordCarto().setFromDegreeGeo(pt1.x ,pt1.y, altitude_sol + hauteur);
                                   var coordCarto2 = new CoordCarto().setFromDegreeGeo(pt2.x ,pt2.y, altitude_sol + hauteur);// + Math.random(1000) );
                                   var coordCarto3 = new CoordCarto().setFromDegreeGeo(pt3.x ,pt3.y, altitude_sol + hauteur);
          ...
          //  gfxEngine().add3DScene(line);
            ...
            var mesh = new THREE.Mesh(new THREE.PlaneGeometry( 10000, 10000, 32 ),new THREE.MeshBasicMaterial( {color: 0xff0000, side: THREE.DoubleSide,transparent: false} ));
            mesh.rotation.x = 0.5;
           // gfxEngine().add3DScene( mesh );
            var sphere = new THREE.Mesh(new THREE.SphereGeometry(500, 20, 20 ),new THREE.MeshBasicMaterial( {color: 0xffff00, side: THREE.DoubleSide, transparent: false, opacity: 0.8} ));
            
            //sphere.position.set(pos.x, pos.y, pos.z);
            mesh.position.set(pos.x+1000,  pos.y+1000, pos.z);
            sphere.position.set(pos.x+1000,  pos.y, pos.z);
            //_currentMeshForClickAndGo.scale.set(1000,1000,1000);
            //_currentMeshForClickAndGo.position.set(pos.x,  pos.y, pos.z);
            
            //{x: -4192856.115042147, y: 4803535.9845473645, z: -170256.208306395}
            console.log("possss: ",pos);
            console.log(sphere);
           // gfxEngine().add3DScene(sphere);//_currentMeshForClickAndGo);
            var axisHelper = new THREE.AxisHelper( 8000000 );
            gfxEngine().add3DScene( axisHelper );
            gfxEngine().add3DScene(_currentMeshForClickAndGo);
            console.log(gfxEngine());
     ...
     */
/*    BatiRGE_Provider.prototype.getTextureBil = function(coWMTS)
    {
